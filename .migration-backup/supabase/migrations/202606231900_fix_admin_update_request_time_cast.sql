-- Fix: admin_update_request fails with 42804
--   "CASE types time without time zone and text cannot be matched"
--
-- A prior revision of this function (the pricing-audit-fields rollout) dropped
-- the ::time cast on the desired_return_time CASE branch:
--
--   desired_return_time = CASE WHEN p_data ? 'desired_return_time'
--                              THEN (p_data->>'desired_return_time')   -- text!
--                              ELSE desired_return_time END             -- time
--
-- The THEN branch is text while ELSE is the time column, so Postgres cannot
-- type the CASE and fails to PLAN the UPDATE — which means EVERY call to
-- admin_update_request errors out, not just the ones that touch return time
-- (assign worker, accept, deny, edit, etc. all break).
--
-- This re-creates the function identically with the ::time cast restored.
