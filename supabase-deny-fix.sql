-- ============================================================
-- supabase-deny-fix.sql
-- Adds cancellation_reason to admin_update_request so the
-- deny flow can use the RPC instead of a direct table update
-- (which is blocked by RLS).
...
BEGIN
  ...
  UPDATE service_requests SET
    ...
    cancellation_reason                = CASE WHEN p_data ? 'cancellation_reason'                THEN (p_data->>'cancellation_reason')                     ELSE cancellation_reason                END,
