...
module.exports = async (req, res) => {
  ...
  if (action === 'notify') {
    ...
    // Awaited so the push actually sends before Vercel freezes the function.
    try { await notifyRequest(body.request_id, body.event); }
    catch (e) { console.warn('[push/notify]', e.message); }
    return res.status(200).json({ ok: true });
  }
