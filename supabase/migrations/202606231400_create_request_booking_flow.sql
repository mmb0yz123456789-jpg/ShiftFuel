-- Consolidated admin-created-request + customer-completion flow.
--
-- Combines what used to be split between archive/sql/supabase-create-request.sql
-- (the CREATE FUNCTION bodies, which were never in a tracked migration) and the
-- search_path / grant tweaks the phase1 lint-cleanup migration applied to these
-- same functions. Having the definitions live only in archive/ meant a fresh
-- `supabase db reset` would NOT create them, and the later config migrations
-- (phase1, advisor_fixes, stragglers) would silently skip the missing functions.
--
-- This migration is now the single source of truth for the three functions
-- that drive the flow:
--   1. admin_create_request      — admin creates a request, no payment yet
--   2. customer_complete_booking — customer fills details + authorizes payment
--   3. public_track_request      — customer looks the request up on the Track page
--
-- search_path is pinned (public, pg_temp) and EXECUTE is granted only to the
-- roles the app actually uses (anon for the browser client; service_role for
-- server/cron), matching the security posture set by the advisor-fix migrations.

begin;

-- ─────────────────────────────────────────────────────────────────────────────
-- 1) Admin: create a service request without collecting payment at creation time
-- ─────────────────────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────────────────────
-- 2) Customer: complete an admin-created booking by adding vehicle + payment info
-- ─────────────────────────────────────────────────────────────────────────────
CREATE OR REPLACE FUNCTION public.customer_complete_booking(
  p_request_id          UUID,
  p_phone               TEXT,
  p_email               TEXT,
  p_vehicle_year        TEXT,
...
-- ─────────────────────────────────────────────────────────────────────────────
-- Grants: this app authenticates with custom session tokens, not Supabase Auth,
-- so EXECUTE is removed from public/authenticated and granted only to the roles
-- the app uses (anon = browser client, service_role = server/cron).
-- ─────────────────────────────────────────────────────────────────────────────
revoke execute on function public.admin_create_request(text, jsonb) from public, authenticated;
grant  execute on function public.admin_create_request(text, jsonb) to anon, service_role;

revoke execute on function public.customer_complete_booking(
  uuid, text, text, text, text, text, text, text, text, text, text, text, text,
  text, text, text, int, boolean, numeric, text, text, text, text, text, text,
  text, text, numeric, numeric, text
) from public, authenticated;
grant  execute on function public.customer_complete_booking(
  uuid, text, text, text, text, text, text, text, text, text, text, text, text,
  text, text, text, int, boolean, numeric, text, text, text, text, text, text,
  text, text, numeric, numeric, text
